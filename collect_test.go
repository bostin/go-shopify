...
func TestCollectList(t *testing.T) {
	...
	httpmock.RegisterResponder("GET", fmt.Sprintf("https://"+testHost+"/%s/collects.json", client.pathPrefix),
...
func TestCollectCount(t *testing.T) {
	...
	httpmock.RegisterResponder("GET", fmt.Sprintf("https://"+testHost+"/%s/collects/count.json", client.pathPrefix),
		httpmock.NewStringResponder(200, `{"count": 5}`))

	params := map[string]string{"since_id": "123"}
	httpmock.RegisterResponderWithQuery("GET",
		fmt.Sprintf("https://"+testHost+"/%s/collects/count.json", client.pathPrefix),
		params,
