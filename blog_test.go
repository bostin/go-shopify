...
func TestBlogList(t *testing.T) {
	...
	httpmock.RegisterResponder(
		"GET",
		fmt.Sprintf("https://"+testHost+"/%s/blogs.json", client.pathPrefix),
...
func TestBlogCount(t *testing.T) {
	...
	httpmock.RegisterResponder(
		"GET",
		fmt.Sprintf("https://"+testHost+"/%s/blogs/count.json", client.pathPrefix),
		httpmock.NewStringResponder(
			200,
			`{"count": 5}`,
		),
...
func TestBlogGet(t *testing.T) {
	...
	httpmock.RegisterResponder(
		"GET",
		fmt.Sprintf("https://"+testHost+"/%s/blogs/1.json", client.pathPrefix),
		httpmock.NewStringResponder(
			200,
			`{"blog": {"id":1}}`,
		),
...
func TestBlogDelete(t *testing.T) {
	...
	httpmock.RegisterResponder("DELETE", fmt.Sprintf("https://"+testHost+"/%s/blogs/1.json", client.pathPrefix),
